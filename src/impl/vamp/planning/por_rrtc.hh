...
namespace vamp::planning
{
    ...
    struct POR_RRTC
    {
        ...
        inline static auto solve(
            ...
            const POR_RRTCSettings &settings,
            typename RNG::Ptr rng,
            Finish_Flag f_finish) noexcept -> PlanningResult<dimension>
        {
            return solve(start, std::vector<Configuration>{goal}, environment, settings, rng, f_finish);
        }
            ...
            Finish_Flag f_finish) noexcept -> PlanningResult<dimension>
        {
            ...
            std::shared_ptr<std::atomic<int>> flag = f_finish.f_ptr;
            PlanningResult<dimension> result;
            if (*flag >= 0) { 
                // std::cout << "finishing suspiciously early" << std::endl;
                return result;
            }
            ...
            //@TODO: only do this if thread_idx = 1
            for (const auto &goal : goals)
            {
                if (validate_motion<Robot, rake, resolution>(start, goal, environment))
                {
                    // Simple check to see if somebody else has finished
                    if (*flag >= 0) { 
                        // std::cout << "finishing suspiciously early" << std::endl;
                        return result;
                    }
                    // more complex check to see if somebody else has finished
                    // if not, claims the flag as its own
                    if (!std::atomic_compare_exchange_strong(flag.get(), &default_flag, settings.thread_id))
                    {
                        // std::cout << "finishing suspiciously early" << std::endl;
                        return result;
                    }
                    ...
                    result.size.emplace_back(1);
                    if (*flag == settings.thread_id)
                    {
                        result.cost = 0;
                    }
                    else
                    {
                        result = PlanningResult<dimension>{};
                        return result;
                    }
                }
            }
            ...
            while (iter++ < settings.max_iterations and free_index < settings.max_samples and *flag < 0)
            {
                        ...
                        environment))
                {
                    ...
                    {
                        // try to set the flag to the current thread id
                        // if it fails, it means another thread has already finished
